...
extension SSHClient {
    /// Begins opening a tunneled TCP/IP channel without awaiting through
    /// `EventLoopFuture.get()`. The initializer still runs after Citadel's
    /// data codec, so it receives `ByteBuffer` inbound values.
    public func createDirectTCPIPChannelFuture(
        using settings: SSHChannelType.DirectTCPIP,
        initialize: @escaping @Sendable (Channel) -> EventLoopFuture<Void>
    ) -> EventLoopFuture<Channel> {
        eventLoop.flatSubmit { [eventLoop, sshHandler = self.session.sshHandler] in
