...
final class SSHClientSession: Sendable {
    ...
    /// Installs the initial SSH pipeline without crossing an async executor.
    /// The caller must already be running on the channel's event loop.
    static func addHandlersSync(
        on channel: Channel,
        inboundChannelHandler: SSHClientInboundChannelHandler,
        settings: SSHClientSettings
    ) throws -> (NIOSSHHandler, ClientHandshakeHandler) {
        ...
        return (sshHandler, handshakeHandler)
    }
